...
## 2.A Ask the user again, whether they want to keep continue or not
if len(unique_species) >1:
    confirmation = input("Your data seems to have more than one species. Would you like to keep analyzing them? (yes or no)\n").strip().lower()
    if confirmation == "yes":
       ...
       print("We successfully download your protein fasta. Check Our Output folder")
    elif confirmation == "no":
       print("You choose no\nThank you for using the tiny protein analyzer. Have a nice day")
       sys.exit()
    else:
       raise ValueError("Invalid input, please type in yes or no")
